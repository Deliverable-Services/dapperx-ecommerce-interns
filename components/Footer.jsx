...
const Footer = () => {
  return (
    <div className="w-full">
      ...
      <div className="flex md:flex-row flex-col md:w-[70%] w-[100%]  justify-around items-center md:items-start md:gap-0 gap-10">
        {FOOTER_LINKS.map((section, index) => {
          return (
            <div key={index} className="gap-3 flex flex-col items-center md:items-start">
              <p className="font-bold text-xl text-green-600">{section.title}</p>
              {section.links.map((subSection) => {
                return (
                  <div key={subSection.id} className="flex flex-col justify-center items-center">
